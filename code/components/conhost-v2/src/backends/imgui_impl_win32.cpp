...
void ImGui_ImplWin32_InitMultiViewportSupport(bool platform_has_own_dc);
...
static bool ImGui_ImplWin32_InitEx(void* hwnd, bool platform_has_own_dc)
{
	...
	const char* xinput_dll_names[] = {
		...
		// CFX: Removed older xinput DLL's as they lack ASLR
...
// CFX: Add Console & Cursor flags to properly ignore mouse input if console is closed.
...
void ImGui_ImplWin32_NewFrame()
{
	...
	RECT rect = { 0, 0, 0, 0 };
	// CFX: Game Clients have issues with this, keep only for FXServer
#ifdef IS_FXSERVER
...
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandlerEx(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, ImGuiIO& io)
{
	...
	switch (msg)
	{
		//CFX: clear keys-down array when losing focus.
...
static void ImGui_ImplWin32_CreateWindow(ImGuiViewport* viewport)
{
	...
	//CFX: Handle RDR3 off-thread window updates.
#ifdef IS_RDR3
	if (vd->HwndParent == ImGui::GetMainViewport()->PlatformHandle)
	{
		vd->HwndParent = NULL;
	}
#endif
	...
	all.cxLeftWidth = -1;
	all.cxRightWidth = -1;
	all.cyBottomHeight = -1;
	all.cyTopHeight = -1;

	DwmExtendFrameIntoClientArea(vd->Hwnd, &all);

	{
		...
		if (hModule)
		{
			...
			if (SetWindowCompositionAttribute)
			{
				...
				SetWindowCompositionAttribute(vd->Hwnd, &wdata);
			}
			FreeLibrary(hModule);
		}
	}
