								  ...
								  _In_opt_ __drv_aliasesMem LPVOID lpParameter, _In_ DWORD dwCreationFlags, _Out_opt_ LPDWORD lpThreadId)
{
	...
	HANDLE hThread = CreateThread(lpThreadAttributes, dwStackSize, [] (void* arguments)
	{
		...
		else
		{
			...
			OPTICK_START_THREAD(va("sysThread (0x%x)", hook::get_unadjusted(meta.originalData->startRoutine)));
		}
...
static HookFunction hookFunction([] ()
{
	...
	void* createThread = hook::pattern("33 C9 44 89 7C 24 20 FF 15").count(1).get(0).get<void>(7);
