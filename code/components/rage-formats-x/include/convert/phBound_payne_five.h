...
namespace rage
{
static inline void fillBaseBound(five::phBound* out, payne::phBound* in)
{
	out->SetBlockMap();
...
static five::phBound* convertBoundToFive(payne::phBound* bound);

template<>
five::phBoundComposite* convert(payne::phBoundComposite* bound)
{
	...
	payne::phBoundAABB* inAABBs = bound->GetChildAABBs();
	...
	payne::Matrix3x4* inMatrices = bound->GetChildMatrices();
...
static inline void fillPolyhedronBound(five::phBoundPolyhedron* out, payne::phBoundPolyhedron* in)
{
	...
	// vertices
	payne::phBoundVertex* vertices = in->GetVertices();
	...
	// polys
	payne::phBoundPoly* polys = in->GetPolygons();
	...
	memcpy(outPolyMaterials.data(), ((payne::phBoundGeometry*)in)->GetPolysToMaterials(), sizeof(uint8_t) * numPolys);

	for (int i = 0; i < numPolys; i++)
	{
		outPolys[i].type = outPolys[i].type & 3;
	}
...
static inline uint8_t ConvertMaterialIndexPayne(uint8_t index)
{
...
static inline void fillGeometryBound(five::phBoundGeometry* out, payne::phBoundGeometry* in)
{
	...
	payne::phBoundMaterial* inMaterials = in->GetMaterials();
	
	for (int i = 0; i < materials.size(); i++)
	{
		materials[i].mat1.materialIdx = ConvertMaterialIndexPayne(inMaterials[i].mat1.materialIdx);
...
template<>
five::phBoundGeometry* convert(payne::phBoundGeometry* bound)
{
...
template<>
five::phBoundBVH* convert(payne::phBoundBVH* bound)
{
...
template<>
five::phBoundSphere* convert(payne::phBoundSphere* bound)
{
...
template<>
five::phBoundBox* convert(payne::phBoundBox* bound)
{
...
template<>
five::phBoundCapsule* convert(payne::phBoundCapsule* bound)
{
...
static five::phBound* convertBoundToFive(payne::phBound* bound)
{
	...
	{
		//case payne::phBoundType::BVH:
		//	return convert<five::phBoundBVH*>(static_cast<payne::phBoundBVH*>(bound));

		case payne::phBoundType::Sphere:
			return convert<five::phBoundSphere*>(static_cast<payne::phBoundSphere*>(bound));

		case payne::phBoundType::Composite:
			return convert<five::phBoundComposite*>(static_cast<payne::phBoundComposite*>(bound));

		case payne::phBoundType::Geometry:
			return convert<five::phBoundGeometry*>(static_cast<payne::phBoundGeometry*>(bound));

		case payne::phBoundType::BVH:
			return convert<five::phBoundBVH*>(static_cast<payne::phBoundBVH*>(bound));

		case payne::phBoundType::Box:
			return convert<five::phBoundBox*>(static_cast<payne::phBoundBox*>(bound));

		case payne::phBoundType::Capsule:
			return convert<five::phBoundCapsule*>(static_cast<payne::phBoundCapsule*>(bound));
	}
...
template<>
five::phBoundComposite* convert(payne::phBound* bound)
{
	if (bound->GetType() == payne::phBoundType::Composite)
	{
...
template<>
five::phBoundComposite* convert(payne::datOwner<payne::phBound>* bound)
{
	return convert<five::phBoundComposite*>(bound->GetChild());
}

template<>
five::phBound* convert(payne::phBound* bound)
{
	return convertBoundToFive(bound);
}

template<>
five::pgDictionary<five::phBound>* convert(payne::pgDictionary<payne::phBound>* phd)
{
