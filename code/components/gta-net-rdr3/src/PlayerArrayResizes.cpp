...
static void updateAllNetworkObjects(void* objMgr)
{
	if (icgi->OneSyncEnabled)
	{
		return;
...
void (*g_unkRemoteBroadcast)(void*, __int64);
void unkRemoteBroadcast(void* a1, __int64 a2)
{
...
static HookFunction hookFunction([]()
{
	...
	// Expand Player Cache data
	{
		...
		//Patch memset size
		//hook::put<int32_t>(hook::get_pattern("41 B8 ? ? ? ? 88 15", 2), kCachedPlayerSize);
	...
	// Replace 32/31 comparisions
	{
		std::initializer_list<PatternClampPair> list = {
			...
			{ "80 7B ? ? 73 ? B2", 3, false },
			//rage::netPlayer::GetPhysicalIndex
	...
	// Forcefully map the struct so scPlayers to be at the old struct end, force the allocator to append another 1024 bits to support 128 players.
	{
		...
		static void** scplayerArray = (void**)hook::AllocateStubMemory((sizeof(void*) * kMaxPlayers + 1));
		memset(scplayerArray, 0, (sizeof(void*) * kMaxPlayers + 1));
		...
		static struct : jitasm::Frontend
 		{
			...
			virtual void InternalMain() override
			{
				mov(r10, reinterpret_cast<uintptr_t>(scplayerArray));
				cmp(qword_ptr[r10 + rax * 8], 0);
...
//				cmp(qword_ptr[rcx + rax * 8 + displacement], rbx);
...

		{
			...
			const auto retnFail = hook::get_pattern("FE C3 80 FB ? 72 ? 33 C0 48 8B 5C 24 ? 48 8B 6C 24 ? 48 8B 74 24 ? 48 83 C4");
			patchStub7.Init((uintptr_t)location + 10, (uintptr_t)retnFail);
	...
	// Patch stack allocation for int[1] that are treated as bitsets, we need it to be int[5] for extended player
	// TODO: Makes script created entities invisible
#if 1
	{
		auto location = hook::get_pattern<intptr_t>("48 81 EC ? ? ? ? 48 83 79 ? ? 48 8B F2 48 8B F9 0F 85");
		// sub rsp, 80h -> sub rsp, 90h
		//hook::put<int32_t>(location + 3, 0x90);
		// ++v29 >= 1 -> ++v29 >= 5
		//hook::put<uint8_t>(hook::get_pattern("48 83 F8 ? 7C ? E9 ? ? ? ? 90", 3), 5);
	}
#endif
	...
	MH_Initialize();
	//INSTALL_HOOK(sub_14259BA94, hook::get_pattern("48 85 D2 74 ? 48 89 5C 24 ? 48 89 6C 24 ? 48 89 74 24 ? 57 48 83 EC ? 33 DB"));

	// Don't run function in onesync. (playerObjects is 32-sized and isn't given information in onesync)
	MH_CreateHook(hook::get_call(hook::get_pattern("E8 ? ? ? ? 44 8B 35 ? ? ? ? 33 C0")), updateAllNetworkObjects, (void**)&g_updateAllNetworkObjects);
#if 1
	...
	//MH_CreateHook(hook::get_pattern("40 53 48 83 EC ? 48 8B D9 E8 ? ? ? ? 48 8B C3 48 83 C4 ? 5B C3 90 8B 05"), getPlayerActiveInfoUnk, (void**)&g_getPlayerActiveInfoUnk);
