...
static void RespawnPlayerPedEvent_Serialize(void* self, rage::CSyncDataBase* syncData)
{
	...
	if (syncData->m_type == rage::SYNC_DATA_READER || syncData->m_type == rage::SYNC_DATA_WRITER)
	{
		...
		trace("RespawnPlayerPedEventSerialize %p | %d %d\n", self, *objectIdRef, *objectId);

		// We don't care about object id mapping.
...
static char GameScriptId_Read(CGameScriptId* self, rage::datBitBuffer* buffer)
{
	...
	//trace("GameScriptId::Read | %p | %d %d | (%d) objectId: %d\n", (void*)self, self->m_scriptHash, self->m_positionHash, self->m_isObjectId, self->m_objectId.m_value);
...
static void NetworkEventComponentControlBase_Write(CNetworkEventComponentControlBase* self, rage::datBitBuffer* buffer)
{
	...
	trace("NetworkEventComponentControlBaseWrite | %d %d %d %d\n", self->m_objectId1, self->m_objectId2, self->unk_C, self->unk_D);
}
...
static void NetworkEventVehComponentControl_Write(CNetworkEventVehComponentControl* self, rage::datBitBuffer* buffer)
{
	...
	trace("NetworkEventVehComponentControlWrite | %d %d %d\n", self->m_occupantObjectId, self->unk_D, self->unk_1A);
...
static void NetworkEventVehComponentControl_WriteInner(CNetworkEventVehComponentControl* self, rage::datBitBuffer* buffer)
{
	...
	trace("NetworkEventVehComponentControlUnk | [objectId: %d] [unk_1A: %d] [unk_E: %d]\n", self->m_occupantObjectId, self->unk_1A, self->unk_E);
...
static uint64_t SyncedObjectId_GetMaxBits(void* self)
{
	if (!icgi->OneSyncEnabled)
	{
		return g_origSyncedObjectId_GetMaxBits(self);
	}
...
static uint64_t SyncedEntity_GetMaxBits(void* self)
{
	if (!icgi->OneSyncEnabled)
	{
		return g_origSyncedEntity_GetMaxBits(self);
	}
...
static uint64_t SyncedConstPed_GetMaxBits(void* self)
{
	if (!icgi->OneSyncEnabled)
	{
		return g_origSyncedConstPed_GetMaxBits(self);
	}
...
static uint64_t SyncedVarGroup_GetMaxBits(void* self)
{
	...
	// We can increase group's size blindly if needed.
	auto result = g_origSyncedVarGroup_GetMaxBits(self);
	return result;
}
...
static HookFunction hookFunction([]()
{
	...
	// Experimental patch for `CSyncedVarGroup`.
	...
	// removing object id mapping in CTaskClimbLadder
	{
		...
		static struct : public jitasm::Frontend
		{
			virtual void InternalMain() override
			{
				// TODO (1207.58 - 0x1422174C7)
				// Ensure to not break non-OneSync mode!
				// 
				// mov edb, edx
				// mov r9d, 0xFFFF
				// ...
			...
			static bool ShouldRemoveIDMapping()
			{
				return icgi->OneSyncEnabled && icgi->OneSyncBigIdEnabled;
			}

			static bool GetIDMapping()
			{
				return false;
			}
		} climbLadderStub;
		hook::call(location, climbLadderStub.GetCode());
#endif
	}
