...
{
public:
	using THandlerPtr = std::shared_ptr<fx::internal::ConsoleVariableEntryBase>;
...
static std::shared_ptr<fx::internal::ConsoleVariableEntry<TValue>> CreateVariableEntry(ConsoleVariableManager* manager, const std::string& name, const TValue& defaultValue)
{
	...
	if (oldEntry)
	{
		...
		auto oldType = std::dynamic_pointer_cast<fx::internal::ConsoleVariableEntry<TValue>>(oldEntry);
		...
		auto newEntry = std::make_shared<fx::internal::ConsoleVariableEntry<TValue>>(manager, name, defaultValue);
